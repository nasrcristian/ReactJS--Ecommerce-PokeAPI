    ...
    return(
        ...
        <div>
            ...
            {pageNumbers.map(number => ( number !== current ?
                <button onClick={()=> paginate(number)} key={number} className="paginationBtn">{number}</button> : <button className="paginationBtn currentBtn" disabled key={number}>{current}</button>))} {/* Uso de ternario para saber siempre en que pagina se encuentra */}
